...
namespace RestClient.Tests
{
    ...
    public class IntegrationTestsFactory
    {
        ...
        private readonly XUnitLogger<BinClientHttpFactory> _logger;
        private BinClientHttpFactory _api;

        public IntegrationTestsFactory(ITestOutputHelper output)
        {
            _httpClientFactory = new TestHttpClientFactory();
            _logger = new XUnitLogger<BinClientHttpFactory>(output);
        }

        [Fact]
        public async Task Get_Guid_Succeeds()
        {
            _api = new BinClientHttpFactory(_httpClientFactory, new RestServiceSettings { BaseAddress = "https://httpbin.org" }, _logger);
        ...
        {
            _api = new BinClientHttpFactory(_httpClientFactory, new RestServiceSettings { BaseAddress = "https://httpbin.org", Authentication = new RestServiceAuthentication { AuthenticationType = ApiAuthenticationType.Basic, UserName = "me", Password = "secret" } }, _logger);
        ...
        {
            _api = new BinClientHttpFactory(_httpClientFactory, new RestServiceSettings { BaseAddress = "https://httpbin.org", Authentication = new RestServiceAuthentication { AuthenticationType = ApiAuthenticationType.External } }, _logger);
